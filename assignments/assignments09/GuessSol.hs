...
prompt :: IO Int
prompt = putStr "Input >>> " >> getLine >>= \inp ->
...
hint :: Int -> Int -> IO ()
hint n guess | n < guess = putStrLn "Your guess is too large!"
             | otherwise = putStrLn "Your guess is too small!"

-- Game loop: ask for guess until game is over, return outcome
gameLoop :: Guess Bool
gameLoop s@(GameState n attempts) 
    | attempts >= 12 = return (False, s)
    | otherwise      = prompt >>= \guess -> 
                       if guess == n 
                        then return (True, s)
                        else hint n guess >> 
                             gameLoop (GameState n $ attempts + 1)
...
main =
    ...
    gameLoop (GameState n 1) >>= \g ->
    -- Print score
    case g of
        (True, (GameState _ as)) -> putStrLn $ "You won within " ++ show as ++ " attempts."
        (False, _) -> putStrLn "You ran out of attempts! Go back to your algorithms class!"
